...
public class SimplifyDebts {
    ...
    public static void main(String[] args) {
        ...
        simplifier.createGraphForDebts();
    }
     ...
     */
    private void createGraphForDebts() {
        ...
        simplifyTransactions(solver);
    }

        public Dinics simplifyTransactions(Dinics solver) {
            Log.d("SimplifyDebts", "Starting debt simplification with " + solver.getEdges().size() + " edges");
            Integer edgePos;
            ...
            while((edgePos = getNonVisitedEdge(solver.getEdges())) != null) {
                ...
                //  Create a new graph
                solver = new Dinics(solver.n, solver.vertexLabels);
            ...
            solver.printEdges();
            Log.d("SimplifyDebts", "Completed debt simplification");
            return solver;
    }
    ...
    private Integer getNonVisitedEdge(List<Dinics.Edge> edges) {
        ...
        for(Dinics.Edge edge : edges) {
            if(!visitedEdges.contains(getHashKeyForEdge(edge.from, edge.to))) {
                edgePos = curEdge;
                break; // Added break to return first non-visited edge
            }
            curEdge++;
        }
        return edgePos;
    ...
    /**
     * Get the simplified debts as a list of Edge objects
     * @param solver the solver after simplification
     * @return list of simplified debt edges
     */
    public List<Dinics.Edge> getSimplifiedDebts(Dinics solver) {
